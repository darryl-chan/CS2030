class WaitEvent extends Event {
    ...
    protected Pair<PQ<Event>, ServerList> update(ServerList sList, PQ<Event> pq) {
        ...
        if (time == this.getTime()) {
            Pair<Server, ServerList> p1 = sList.verify(this.getServer(), time);

            //Server s = sList.verify(this.getServer(), time);
            double wait = time - this.initial;
            Event event = new ServeEvent(this.getCustomer(), time, p1.first(), wait);

            pq = pq.add(event);
            return new Pair<PQ<Event>, ServerList>(pq, p1.second());
        } else {
